...
{
    if(len < UART_SERVER_MAX_MTU)
    {
        memcpy(s_ble_to_uart_buff,(uint8_t*)value,len);
        uart_tx_it(s_ble_to_uart_buff,len);
    }
}
...
void ble_master_recv_data_ind(uint8_t con_idx, uint8_t const *value, uint16_t len)
{
    if(len < UART_SERVER_MAX_MTU)
    {
        memcpy(m_ble_to_uart_buff,(uint8_t*)value,len);
        uart_tx_it(m_ble_to_uart_buff,len);
    }
}
