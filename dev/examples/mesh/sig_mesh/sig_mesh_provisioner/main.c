...
uint16_t node_unicast_address= NODE_BASE_UNICAST_ADDR;
...
static void prover_mesh_manager_callback(enum mesh_provisioner_evt_type type, union ls_sig_mesh_provisioner_evt_u *evt)
{
    ...
    case MESH_PROVER_HEALTH_MODEL_RSP_INFO:
    {
       health_client_model_lid =  evt->prover_node_health_model_rsp_info.mdl_lid;
        ...
        if (tinyfs_read(ls_sigmesh_provision_dir, RECORD_PROVER_NETKEY_LID, &netkey_lid, &lid_length) !=TINYFS_NO_ERROR)
        {
           ls_sig_mesh_provisioner_add_net_key(NETKEY_ID0,&netkey[0][0]);  //net_id >2
        }
        else
        {
             if (tinyfs_read(ls_sigmesh_provision_dir, RECORD_PROVER_APPKEY_LID, &appkey_lid, &lid_length) !=TINYFS_NO_ERROR)
             {
    ...
    case MESH_PROVER_KEY_NET_ADD_IND:
    {
        ...
        tinyfs_write(ls_sigmesh_provision_dir, RECORD_PROVER_NETKEY_LID, &netkey_lid, sizeof(uint8_t));
     ...
     case MESH_PROVER_KEY_APP_ADD_IND:
    {
        ...
        tinyfs_write(ls_sigmesh_provision_dir, RECORD_PROVER_APPKEY_LID, &appkey_lid, sizeof(uint8_t));
        tinyfs_write_through();
    }
    break;
    ...
    {
        node_unicast_address += node_elt.max_num_elements; 
        ls_sig_mesh_identify_cfm(true /*accept*/,netkey_lid/*netkey_lid*/,node_unicast_address/*dev primary unicast address*/,0x00/*FIPS P-256 ECC*/,0x00/*No OOB Public key uesed*/,0x00/*auth_method*/,0x00/*auth_action*/,0x00/*auth_size*/);
    }
    break; 
    default:
       break;
