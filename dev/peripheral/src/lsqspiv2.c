...
XIP_BANNED void lsqspiv2_init(void)
{
    ...
    REG_FIELD_WR(LSQSPIV2->QSPI_CTRL1,LSQSPIV2_CAP_DLY, QSPI_CAPTURE_DELAY);
}

XIP_BANNED static void lsqspiv2_xip_mode_start()
{
...
XIP_BANNED void lsqspiv2_direct_quad_read_config()
{
...
XIP_BANNED void lsqspiv2_direct_dual_read_config()
{
    LSQSPIV2->DAC_CTRL = FIELD_BUILD(LSQSPIV2_DAC_HZ_CYC,0)|FIELD_BUILD(LSQSPIV2_DAC_MW_WID,DUAL_WIRE)|FIELD_BUILD(LSQSPIV2_DAC_MW_CYC,15)
        | FIELD_BUILD(LSQSPIV2_DAC_MW_EN,1)|FIELD_BUILD(LSQSPIV2_DAC_SW_EN,0);
    lsqspiv2_xip_mode_start();
}

XIP_BANNED static void write_data_to_fifo(struct lsqspiv2_stg_cfg *cfg)
{
...
XIP_BANNED void lsqspiv2_stg_send_command(uint8_t opcode)
{
...
XIP_BANNED void lsqspiv2_stg_read_register(uint8_t opcode,uint8_t *data,uint8_t length)
{
    struct lsqspiv2_stg_cfg cfg;
    cfg.ctrl.sw_cyc = 7;
    cfg.ctrl.sw_en = 1;
...
XIP_BANNED void lsqspiv2_stg_write_register(uint8_t opcode,uint8_t *data,uint8_t length)
{
    struct lsqspiv2_stg_cfg cfg;
    cfg.ctrl.sw_cyc = 7;
    cfg.ctrl.sw_en = 1;
